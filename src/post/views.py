...
async def comment_on_post(post_id: int, content: str, token: str, db: Session = Depends(get_db)):
    # verify the token
    user = await get_current_user(db, token)
    if not user:
        raise HTTPException(
    ...
    res, detail = await comment_on_post_svc(db, post_id, user.id, content)
    if not res:
...
async def get_comments_for_post(post_id: int, db: Session = Depends(get_db)):
    comments = await get_comments_for_post_svc(db, post_id)
    if not comments:
