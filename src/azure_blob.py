...
async def upload_to_azure_blob(file: UploadFile, username: str, user_id: str) -> tuple:
    ...
    try:
        ...
        return blob_url  # Return both the URL and the media type
        ...
        media_url = f"{CDN_BASE_URL}/{container_name}/{blob_name}"
        ...
        if media_type == "video":
            ...
            temp_thumb.close()  # close temp file before writing
            ...
            thumbnail_url = f"{CDN_BASE_URL}/{AZURE_IMAGE_CONTAINER}/{thumb_blob_name}"

            # ✅ Delay + remove after ensuring no handle is locking it
            sleep(0.1)
