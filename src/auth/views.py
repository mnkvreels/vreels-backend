...
def otp_function(db, user_id, phone_number):
...
async def send_otp(user: UserUpdate, db: Session = Depends(get_db)):
    ...
    db_user = authenticateMobile(db, user.phone_number)
    
    if db_user:
        # If user exists, generate OTP for the existing user and send it
        otp = otp_function(db, db_user.id, user.phone_number)
        sms_status = send_sms(user.phone_number, otp)
...
from sqlalchemy.orm import Session
from src.database import get_db  # Ensure this function is correctly defined

@router.post("/verify-otp", status_code=status.HTTP_200_OK)
async def verify_otp(user_id: int, otp: str, db: Session = Depends(get_db)):
    otp_record = db.query(OTP).filter(OTP.user_id == user_id, OTP.otp == otp).first()
    return {"message": "OTP verified"} if otp_record else {"error": "Invalid OTP"}
    ...
    db_user = authenticateUserID(db, user_id)
